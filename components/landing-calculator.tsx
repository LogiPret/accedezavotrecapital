...
interface LandingCalculatorProps {
  embedded?: boolean; // When true, renders just the card without section wrapper
}
...
export default function LandingCalculator({
  ...
  const handleUnlock = async () => {
    ...
    if (!unlockEmail.trim() || !emailRegex.test(unlockEmail)) {
      setUnlockError(t.landing.errorEmail);
      trackLandingUnlockError("invalid_email");
      return;
    }
    ...
    <Card className="shadow-xl border-0 overflow-hidden p-0">
      ...
      <CardContent className="p-4 md:p-6 lg:p-8">
        <div className="grid lg:grid-cols-2 gap-6 md:gap-8">
            ...
            <div className="space-y-2 md:space-y-3">
              ...
              {hasSpouse && (
                <div className="pl-6 md:pl-7 space-y-2 md:space-y-3">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="spouseAge" className="text-xs md:text-sm">
                ...
                <div className="space-y-1 md:space-y-2">
                  ...
                  >
                    <SelectTrigger id="city" className="text-sm">
                      <SelectValue placeholder={t.calculator.cityPlaceholder} />
                    </SelectTrigger>
                    <SelectContent>
          ...
          {/* Results Section with Unlock Requirement */}
          <div className="relative">
            {/* Show unlock form if results requested but not unlocked */}
            {showResults && !isUnlocked ? (
                  ...
                  >
                    {isUnlocking ? (
                      <span className="flex items-center gap-2">
                        ...
                        {locale === "en" ? "Unlocking..." : "Déverrouillage..."}
                      </span>
                    ) : (
              ...
              /* Show results only when unlocked */
              <div
                    ...
                    <>
                      ...
                      <div className="p-3 md:p-4 bg-primary/5 rounded-lg border border-primary/10">
                        <p className="text-[10px] md:text-xs text-muted-foreground flex items-start gap-2">
                          <Info className="w-3 h-3 md:w-4 md:h-4 shrink-0 mt-0.5 text-primary" />
  ...
  // When embedded, just return the card without section wrapper
  if (embedded) {
    return (
      <div id="calculatrice" className="max-w-4xl mx-auto w-full">
        {calculatorCard}
      </div>
    );
  }
